(function ($) {
    ...
    _ox, _oy; // 拖动时，保存起点位置;
        ...
        var clickHandler = function (e, src) {
            ...
            if (src.getId() == _id) {
                $(_r).trigger('showprops', ["node",_o.props, src]);
            } else {
                hideBox();
            }
        };
        ...
        this.restore = function (data) {
			...
			var nowPropId="";
			for (var k in data.props) {
				if(k == "text"){
					continue;
				}
				nowPropId=this.getId()+"_"+k      
				$('#'+nowPropId).val(data.props[k].value);
            }	
    ...
    myflow.path = function (o, r, from, to, guid, ec,dots,id) {
        ...
        var clickHandler = function (e, src) {
            ...
            if (src && src.getId() == _id) {
                _dotList.show();
                $(_r).trigger('showprops', ["path",_o.props, _this]);
            } else {
                _dotList.hide();
            }
        ...
        this.restore = function (data) {
            var obj = data;
            _o = $.extend(true, _o, data);
			if(data.props.text&&data.props.text.value&&data.props.text.value!=''){
				_o.text.text = data.props.text.value;
			}
    ...
    myflow.props = function (o, r) {
        ...
        var showpropsHandler = function (e, type,props, src) {
			...
			var ahtml = "<div style=\"width:100%;text-align:center\" id=\"pextendInfoLinkDiv\" class=\"editor\"></div>";
			_tb.append('<tr><th colspan="2">'+ahtml+'</th></tr>');
			var extendInfoEditor =  new myflow.editors.extendEditor();
			extendInfoEditor.init(props,"extendInfoLinkDiv","pextendInfoLinkDiv",src,_r,type);
        };
    ...
    myflow.init = function (c, o) {
        var _w = $(window).width(), _h = $(window).height(), _r = Raphael(c, _w
                    * 1.5, _h * 1.5), _states = {}, _paths = {}, _infos = {};
        ...
        */
        ...
        $(document).click(function () {
            ...
            $(_r).trigger('showprops', ["flow",myflow.config.props.props, {
                getId: function () {
                    return '00000000';
                }
            }]);
        ...
        if (myflow.config.editable) {
            ...
            $('#myflow_save').click(function () {// 保存
				var nowData = getJson();
                myflow.config.tools.save(nowData)
            });
            $('#myflow_publish').click(function () {// 发布
                myflow.config.tools.moreInfoClick(getJson())
            });
        ...
        function restore(data){
			...
			if(data.infos){				
				for (var k in data.infos) {
					myflow.config.props.props[k].value = data.infos[k];
                }
			}
        }
